...
本分支通过 51 项确定性测试，以及最终速度修正后的 Gate H：
...
1. Tier B：连续可信遮罩、雷达离散归属、已确认墙定位校正与防自证；
   用 Observation Adapter 接入，不能把 Tier A 的真值语义传感当成实车证据。
2. 实装按未知决策边界距离控制速度的公式（规范 §10），再接
   RealObservationAdapter / RealMotionAdapter；官方 `car/` 保持独立。
3. 在真实 M3 Pro 上量测固定模板的车体净空、轮速接缝与定位误差，
   完成规范中超出本轮语义模拟的物理 Gate。

## 验收边界

本轮可声称的是 Tier A 语义模拟的拓扑、任务与几何契约。
真实雷达关联、闭环定位、车体 swept-OBB 最小净空、轮速 PID 和实车运行
尚未验收；`SemanticSim` 通过不等于机器人已能安全上场。
